...
using D3BEmu.Net.GS.Message.Definitions.Conversation;
...
namespace D3BEmu.Core.GS.Players
{
    ...
    class Conversation
    {
        ...
        private D3BEmu.Common.MPQ.FileFormats.Conversation asset;
        ...
        private D3BEmu.Core.GS.Actors.Actor GetSpeaker(Speaker speaker)
        {
            switch (speaker)
            {
        ...
        private D3BEmu.Core.GS.Actors.Actor GetActorBySNO(int sno)
        {
            var actors = (from a in player.RevealedObjects.Values where a is D3BEmu.Core.GS.Actors.Actor && (a as D3BEmu.Core.GS.Actors.Actor).ActorSNO.Id == sno select a);
            if (actors.Count() > 1)
            ...
            return actors.First() as D3BEmu.Core.GS.Actors.Actor;
        }
        ...
        {
            asset = D3BEmu.Common.MPQ.MPQStorage.Data.Assets[Common.Types.SNO.SNOGroup.Conversation][snoConversation].Data as D3BEmu.Common.MPQ.FileFormats.Conversation;
            this.player = player;
        ...
        public void Update(int tickCounter)
        {
            ...
            else
            {
                ...
                {
                    D3BEmu.Core.GS.Actors.Actor speaker1 = GetSpeaker(currentLineNode.Speaker1);
                    D3BEmu.Core.GS.Actors.Actor speaker2 = GetSpeaker(currentLineNode.Speaker2);
    ...
    public class ConversationManager
    {
        ...
        {
            if (!D3BEmu.Common.MPQ.MPQStorage.Data.Assets[Common.Types.SNO.SNOGroup.Conversation].ContainsKey(snoConversation))
            {
        ...
        public void Consume(GameClient client, GameMessage message)
        {
            lock (openConversations)
            {
                // Reuse RequestCloseConversationWindowMessage handling for AdvanceConvMessage for now since AdvanceConvMessage doesn't contain a separate field for conversation SNO in 7447
                if (message is RequestCloseConversationWindowMessage || message is AdvanceConvMessage)
                {
                    List<Conversation> clonedList;
