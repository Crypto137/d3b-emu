...
using D3BEmu.Net.GS.Message.Definitions.Attribute;
using System.Linq;
        ...
        */
        ...
        private List<GameMessage> GetMessageListFromEnumerator(IEnumerator<KeyId> e, int count, bool level)
        {
            ...
            if (count == 1)
            {
                ...
                msg.Field1 = new D3BEmu.Net.GS.Message.Fields.NetAttributeKeyValue();
            ...
            else
            {
                ...
                if (count >= 15)
                {
                    for (; count >= 15; count -= 15)
                    {
                        ...
                        msg.ActorID = _parent.DynamicID;
                        msg.atKeyVals = new D3BEmu.Net.GS.Message.Fields.NetAttributeKeyValue[15];
                        for (int i = 0; i < 15; i++)
                            msg.atKeyVals[i] = new D3BEmu.Net.GS.Message.Fields.NetAttributeKeyValue();
                        for (int i = 0; i < 15; i++)
                        {
                            KeyId keyid;
                            if (!e.MoveNext())
                ...
                if (count > 0)
                {
                    AttributesSetValuesMessage msg = new AttributesSetValuesMessage();
                    msg.ActorID = _parent.DynamicID;
                    msg.atKeyVals = new D3BEmu.Net.GS.Message.Fields.NetAttributeKeyValue[count];
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        var kv = new D3BEmu.Net.GS.Message.Fields.NetAttributeKeyValue();
                        msg.atKeyVals[i] = kv;
