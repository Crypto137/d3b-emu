...
using World = D3BEmu.Core.GS.Map.World;
using Scene = D3BEmu.Core.GS.Map.Scene;
using D3BEmu.Core.GS.Common.Types.Scene;

namespace D3BEmu.Core.GS.Generators
{
    public static class WorldGenerator
    {
        ...
        public static World Generate(Game game, int worldSNO)
        {
            ...
            var worldData = (D3BEmu.Common.MPQ.FileFormats.World)worldAsset.Data;
            ...
            var clusters = new Dictionary<int, D3BEmu.Common.MPQ.FileFormats.SceneCluster>();
            ...
            var clusterSelected = new Dictionary<int, List<D3BEmu.Common.MPQ.FileFormats.SubSceneEntry>>();
            foreach (var cID in clusterCount.Keys)
            {
                var selected = new List<D3BEmu.Common.MPQ.FileFormats.SubSceneEntry>();
            ...
            foreach (var sceneChunk in worldData.SceneParams.SceneChunks)
            {
                ...
                {
                    MiniMapVisibility = Map.SceneMiniMapVisibility.Revealed,
                ...
                if (sceneChunk.SceneSpecification.ClusterID != -1)
                {
                    ...
                    else
                    {
                        ...
                        D3BEmu.Common.MPQ.FileFormats.SubSceneEntry subSceneEntry = null;

                        if (entries.Count > 0)
                        {
                            //subSceneEntry = entries[RandomHelper.Next(entries.Count - 1)];

                            subSceneEntry = RandomHelper.RandomItem<D3BEmu.Common.MPQ.FileFormats.SubSceneEntry>(entries, entry => 1); // TODO Just shuffle the list, dont random every time. /fasbat
        ...
        private static void GenerateRandomDungeon(int worldSNO, D3BEmu.Common.MPQ.FileFormats.World worldData)
        {
            ...
            for (int paramIndex = 0; paramIndex < worldData.DRLGParams.Count; paramIndex++)
            {
                ...
                //    [11]: {[161961, D3BEmu.Common.MPQ.MPQAsset]}Worlds\\a1trDun_Cave_Old_Ruins_Random01.wrl
                if (worldSNO == 161961)
                {
        ...
        private static void ProcessCommands(DRLGParams drlgParams, D3BEmu.Common.MPQ.FileFormats.World worldData, int levelIndex)
        {
            //Process commands
            foreach (var command in drlgParams.Commands)
            {
                ...
                {
                    //  command.TagMap
                    //{D3BEmu.Core.GS.Common.Types.TagMap.TagMap}
        ...
        private static void AddTile(D3BEmu.Common.MPQ.FileFormats.World worldData, TileInfo tileInfo, Vector3D location)
        {
            ...
            var sceneData = (D3BEmu.Common.MPQ.FileFormats.Scene)sceneFile.Data;
        ...
        private static void loadLevelAreas(Dictionary<int, List<Scene>> levelAreas, World world)
        {
            ...
            // Each monster are created in D3BEmu.Core.GS.Actors.Implementations.Monsters
            // By Poluxxx
            //int[] aSNO = new int[] {
            //       // 6652      // Zombie
            //         6443      // Ravenous
            //        //, 136943    // Ghost
            //};
            Dictionary<PRTransform, D3BEmu.Common.MPQ.FileFormats.Actor> dict = new Dictionary<PRTransform, D3BEmu.Common.MPQ.FileFormats.Actor>();
                        ...
                        for (; amount > 0; amount--)
                        {
                            ...
                            switch (spawnEntry.SNOHandle.Group)
                            {
                                case SNOGroup.Actor:
                                    ...
                                    loadActor(spawnEntry.SNOHandle, gizmoLocations[location], world, ((D3BEmu.Common.MPQ.FileFormats.Actor)spawnEntry.SNOHandle.Target).TagMap);
                                    break;
                                case SNOGroup.Encounter:
                                    ...
                                    loadActor(actorHandle, gizmoLocations[location], world, ((D3BEmu.Common.MPQ.FileFormats.Actor)actorHandle.Target).TagMap);
                                    break;
                ...
                foreach (var scene in levelAreas[la])
                {
                    ...
                    for (int i = 0; i < 100; i++)
                    {
                        if (RandomHelper.NextDouble() > 0.8)
                        {
                            ...
                            if ((scene.NavMesh.Squares[y * scene.NavMesh.SquaresCountX + x].Flags & D3BEmu.Common.MPQ.FileFormats.Scene.NavCellFlags.NoSpawn) == 0)
                            {
                                loadActor(
        ...
        {
            var actor = D3BEmu.Core.GS.Actors.ActorFactory.Create(world, actorHandle.Id, tagMap);
        ...
        /// </summary>
        private static Vector3D FindSubScenePosition(D3BEmu.Common.MPQ.FileFormats.SceneChunk sceneChunk)
        {
            var mpqScene = MPQStorage.Data.Assets[SNOGroup.Scene][sceneChunk.SNOHandle.Id].Data as D3BEmu.Common.MPQ.FileFormats.Scene;

            foreach (var markerSet in mpqScene.MarkerSets)
            {
                var mpqMarkerSet = MPQStorage.Data.Assets[SNOGroup.MarkerSet][markerSet].Data as D3BEmu.Common.MPQ.FileFormats.MarkerSet;
                foreach (var marker in mpqMarkerSet.Markers)
                    if (marker.Type == D3BEmu.Common.MPQ.FileFormats.MarkerType.SubScenePosition)
