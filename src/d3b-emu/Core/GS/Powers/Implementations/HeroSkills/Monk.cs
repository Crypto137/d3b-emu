...
using D3BEmu.Net.GS.Message.Definitions.Actor;
...
{
    #region Spirit Generators

    // 9359 implementation
    //TODO Rune_A,B,C
    #region FistsOfThunder
    [ImplementsPowerSNO(Skills.Skills.Monk.SpiritGenerators.FistsOfThunder)]
    public class FistsOfThunder : ComboSkill
    {
        public override IEnumerable<TickTimer> Main()
        {
            switch (TargetMessage.ComboLevel)
            {
                case 0:
                    if (Rune_A > 0)
                    {
    ...
    //Complete
    #region DeadlyReach
    [ImplementsPowerSNO(Skills.Skills.Monk.SpiritGenerators.DeadlyReach)]
    public class DeadlyReach : ComboSkill
    {
        public override IEnumerable<TickTimer> Main()
        {
            float reachRadius;
    ...
    [ImplementsPowerSNO(Skills.Skills.Monk.SpiritGenerators.CripplingWave)]
    public class CripplingWave : ComboSkill
    {
        public override IEnumerable<TickTimer> Main()
        {
            int effectSNO;
    ...
    [ImplementsPowerSNO(Skills.Skills.Monk.SpiritGenerators.ExplodingPalm)]
    public class ExplodingPalm : ComboSkill
    {
        public override IEnumerable<TickTimer> Main()
        {
            AttackPayload attack = new AttackPayload(this);
    ...
    #region SweepingWind
    [ImplementsPowerSNO(Skills.Skills.Monk.SpiritGenerators.SweepingWind)]
    public class SweepingWind : ComboSkill
    {
    ...
    [ImplementsPowerSNO(Skills.Skills.Monk.SpiritGenerators.WayOfTheHundredFists)]
    public class WayOfTheHundredFists : ComboSkill
    {
        public override IEnumerable<TickTimer> Main()
        {
            switch (TargetMessage.ComboLevel)
    ...
    public class LethalDecoy : Skill
    {
        public override IEnumerable<TickTimer> Main()
        {
            ...
            // Minion spawning based on Wizard Mirror Image for now
            LethalDecoyMinion decoy = new LethalDecoyMinion(this.World, this, 0);

            decoy.Brain.DeActivate();
            decoy.Position = RandomDirection(User.Position, 3f, 8f);
            decoy.Attributes[GameAttribute.Untargetable] = true;
            decoy.EnterWorld(decoy.Position);
            yield return WaitSeconds(1f);

            decoy.Brain.Activate();
            decoy.Attributes[GameAttribute.Untargetable] = false;
            decoy.Attributes.BroadcastChangedIfRevealed();
