...
namespace D3BEmu.Core.GS.Powers
{
    public class PowerContext
    {
        ...
        public void TraceScriptFormulaValues()
        {
            string formulaValuesString = $"Formula values for power SNO {PowerSNO}: ";

            for (int i = 0; i < 40; i++)
            {
                formulaValuesString = $"{formulaValuesString}{i} = {ScriptFormula(i)}    ";
            }

            Logger.Trace(formulaValuesString);
        }
